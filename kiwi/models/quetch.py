...
class QUETCH(Model):
    ...
    def _build_embeddings(self, source_vectors=None, target_vectors=None):
        ...
        if source_vectors is not None:
            ...
            '''start'''
            # self.source_bert_emb = nn.Embedding(
            #     num_embeddings=self.config.source_vocab_size,
            #     embedding_dim=1024,
            #     padding_idx=self.config.source_padding_idx,
            #     # _weight=self.source_bert
            # )
            # self.source_bert_emb.weight.data.copy_ = self.source_bert
            # self.source_bert_emb.weight.requires_grad = True
            '''end'''
        ...
        if target_vectors is not None:
            ...
            '''start'''
            # self.target_bert_emb = nn.Embedding(
            #     num_embeddings=self.config.target_vocab_size,
            #     embedding_dim=768,
            #     padding_idx=self.config.target_padding_idx,
            #     # _weight=self.target_bert
            # )
            # self.target_bert_emb.weight.data.copy_ = self.target_bert
            # self.target_bert_emb.weight.requires_grad = True
            '''end'''
