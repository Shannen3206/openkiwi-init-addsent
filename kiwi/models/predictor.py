...
class Predictor(Model):
    ...
    def __init__(self, vocabs, **kwargs):
        ...
        """start:加载预训练权重"""
        source_vectors = getattr(vocabs['source'], 'vectors')
        target_vectors = getattr(vocabs['target'], 'vectors')
        """end"""
        ...
        if source_vectors is not None:
            ...
            self.lstm_source = nn.LSTM(
                input_size=source_vectors.size(1),
        ...
        if target_vectors is not None:
            ...
            self.forward_target = nn.LSTM(
                input_size=target_vectors.size(1),
                hidden_size=self.config.hidden_pred,
                num_layers=self.config.rnn_layers_pred,
                batch_first=True,
                dropout=self.config.dropout_pred,
        ...
        if target_vectors is not None:
            self.V = nn.Parameter(
                torch.zeros(2 * target_vectors.size(1), 2 * self.config.out_embeddings_size)
            )
        else:
            self.V = nn.Parameter(
                torch.zeros(2 * self.config.target_embeddings_size, 2 * self.config.out_embeddings_size)
            )
        self.C = nn.Parameter(
            torch.zeros(2 * self.config.hidden_pred, 2 * self.config.out_embeddings_size)
        )
        self.S = nn.Parameter(
            torch.zeros(
