...
class Trainer:
    ...
    def train_epoch(self, train_iterator, valid_iterator):
        ...
        for i, batch in enumerate(tqdm(train_iterator, total=len(train_iterator), desc='Batches', unit=' batches', ncols=80)):
            # train_memory = train_memory_all[i]
            self._step += 1
    ...
    def save(self, output_directory):
        ...
        model_path = output_directory / const.MODEL_FILE    # model.torch
        self.model.save(str(model_path))

        optimizer_path = output_directory / const.OPTIMIZER     # optim.torch
        scheduler_dict = None
        if self.scheduler:
            scheduler_dict = {
        ...
        state_path = output_directory / const.TRAINER       # trainer.torch
