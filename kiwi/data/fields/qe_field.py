    ...
    def build_vocab(self, *args, **kwargs):
        ...
        for arg in args:    # arg是QEDataset类，里面包括examples和fields
            if isinstance(arg, data.Dataset):
                sources += [getattr(arg, name) for name, field in arg.fields.items() if field is self]  # source是列表，列表中元素是迭代器
            else:
                sources.append(arg)
        ...
        specials = list(
            OrderedDict.fromkeys(
                tok for tok in [self.unk_token, self.pad_token, self.init_token, self.eos_token, self.unaligned_token]
                if tok is not None
            )
        )       # ['<unk>', '<pad>', '<unaligned>']
