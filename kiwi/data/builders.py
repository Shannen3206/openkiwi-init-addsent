...
def build_dataset(fieldset, prefix='', filter_pred=None, **kwargs):
    ...
    dataset = QEDataset(examples=examples, fields=fields, filter_pred=filter_pred)
    return dataset
...
def build_training_datasets(
    ...
    **kwargs,
    ...
    """
    # TODO: improve handling these length options (defaults are set multiple times).
    # 过滤器
    filter_pred = partial(
        filter_len,
    ...
    # 构造训练数据和验证数据集，其中包括examples和fields，但fields中的vocab还没有构建好
    train_dataset = build_dataset(fieldset, prefix=Fieldset.TRAIN, filter_pred=filter_pred, **kwargs)
    if valid_source and valid_target:
        valid_dataset = build_dataset(
            fieldset, prefix=Fieldset.VALID, filter_pred=filter_pred, valid_source=valid_source, valid_target=valid_target,
            **kwargs,
        )
    elif split:
        if not 0.0 < split < 1.0:
            raise Exception('Invalid data split value: {}; it must be in the (0, 1) interval.'.format(split))
    ...
    # Even if vocab is loaded, we need to build the vocabulary in case fields are missing
    ...
    # vocab域的配置选项，包括vectors_fn/min_freq/max_size/rare_with_vectors/add_vectors_vocab
