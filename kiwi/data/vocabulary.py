...
class Vocabulary(torchtext.vocab.Vocab):
    ...
    def __init__(
        ...
        '''words_and_frequencies是从语料中抽取出来的单词和词频，vectors是从外部获取的词向量，最终生成的词表保存在self中'''
        words_and_frequencies = sorted(counter.items(), key=lambda tup: tup[0])     # 根据字母排序
        words_and_frequencies.sort(key=lambda tup: tup[1], reverse=True)            # 根据频率排序
        ...
        # vocabulary
        for word, freq in words_and_frequencies:    # 这里的单词是根据语料抽取出来的
            if freq < min_freq:                 # 词频小于最小频率
                if vectors is not None and rare_with_vectors:
                    ...
                    break
            elif len(self.itos) == max_size:    # 词表达到最大长度
                break
            else:
                self.itos.append(word)

        if add_vectors_vocab:   # 将外部词表中多出来的单词也添加进来
            if max_size is not None and sum(v.stoi for v in vectors) + len(self.itos) > max_size:
                warnings.warn('Adding the vectors vocabulary will make len(vocab) > max_vocab_size!')
            vset = set()
            for v in vectors:
        ...
        self.stoi = defaultdict(_default_unk_index)     # 开始更新self.stoi
        # stoi is simply a reverse dict for itos
        self.stoi.update({tok: i for i, tok in enumerate(self.itos)})

        self.vectors = None     # 更新self.vectors
