...
class Corpus:
    ...
    def examples_per_field(self):
        examples = {
            field: examples for (field, _), examples in zip(self.dataset_fields, self.fields_examples)
        }
        return examples
    ...
    def from_files(cls, fields, files):
        ...
        Arguments:
            ...
            files: A dict between field name and file dict (with 'name' and
                   'format' keys).
        """
        fields_examples = []    # 存放examples
        dataset_fields = []     # 存放数据域
        ...
        nb_lines = [len(fe) for fe in fields_examples]      # fields_examples中包含source/target/alignment/tags
