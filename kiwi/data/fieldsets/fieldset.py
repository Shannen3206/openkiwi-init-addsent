...
class Fieldset:
    ...
    def fields_and_files(self, set_name, **files_options):
        fields = {}
        files = {}
        for name, file_option_suffix in self._options.items():          # name: 'source'; file_option_suffix: '_source'
            file_option = '{}{}'.format(set_name, file_option_suffix)   # 文件名 'train_source'
            file_name = files_options.get(file_option)                  # 文件地址
            if not file_name and self.is_required(name, set_name):
                raise FileNotFoundError(
                    'File {} is required (use the {} option).'.format(file_name, file_option.replace('_', '-'))
                )
            elif file_name:
                files[name] = {
                    'name': file_name,      # 文件地址
                ...
                fields[name] = self._fields[name]   # 域
        return fields, files
    ...
    def vocab_vectors_loader(self, name, embeddings_format='polyglot', embeddings_binary=False, **kwargs):
        ...
        if option_name:
            ...
            if option_value:
                ...
                logger.info('Loading {} embeddings from {}'.format(name, option_value))
