...
def serialize_vocabs(vocabs, include_vectors=True):
...
def load_vocabularies_to_fields(vocab_path, fields):
    ...
    if Path(vocab_path).exists():
        vocabs_dict = torch.load(str(vocab_path), map_location=lambda storage, loc: storage)
        ...
        return all([vocab_loaded_if_needed(field) for _, field in fields.items()])
    return False
...
def filter_len(
    ...
    return (source_min_length <= len(x.source) <= source_max_length) and (target_min_length <= len(x.target) <= target_max_length)
