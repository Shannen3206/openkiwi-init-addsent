...
def run(ModelClass, output_dir, pipeline_options, model_options):
    ...
    save_vocabularies_from_datasets(output_dir, *datasets)      # 保存词表
    ...
    trainer = retrieve_trainer(ModelClass, pipeline_options, model_options, vocabs, output_dir, device_id)
    ...
    # import os
    # path = os.getcwd()
    # train_memory_all, dev_memory_all = [], []
    # with open(path+'/data/opennmt-exp3/train.pkl', 'rb') as f:
    #     while True:
    #         try:
    #             train_memory_all.append(pkl.load(f))
    #         except EOFError:
    #             break
    # with open(path+'/data/opennmt-exp3/dev.pkl', 'rb') as f:
    #     while True:
    #         try:
    #             dev_memory_all.append(pkl.load(f))
    #         except EOFError:
    #             break
...
def retrieve_datasets(fieldset, pipeline_options, model_options, output_dir):
    ...
    if pipeline_options.load_data:
        ...
        datasets = builders.build_training_datasets(fieldset, load_vocab=load_vocab, **vars(model_options))
    return datasets
...
def log(output_dir, config_options, config_file_name="train_config.yml", save_config=None):
    """
